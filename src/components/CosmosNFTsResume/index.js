        ...
        <div className="resumen-list-head">
          ...
          {purchasedItems.map((boughtItem, index) => (
            <div className='resumen-list-body' key={index}>
              ...
              <p className="resumen-list-body__item">{boughtItem.name ? boughtItem.name.slice(0, 4) + '...' : ''}</p>
              <p className="resumen-list-body__item">   {parseInt(boughtItem.price) / 1000000000000000000}</p>
              <p className="resumen-list-body__show" onClick={() => {onShowDetail(boughtItem);}}>ver</p>
              <a className="resumen-list-body__address" href={`https://testnet.snowtrace.io/address/${boughtItem.buyer}`}> {boughtItem.buyer.slice(0, 4) + "..." + boughtItem.buyer.slice(38)}</a>
            </div>
          ))}
